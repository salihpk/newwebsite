...
function App() {
    ...
    useEffect(() => {
        const detectVPN = async () => {
            ...
            const apis = [
                {
                    url: 'http://ip-api.com/json/?fields=status,message,city,regionName,query,org',
                    parser: (d) => d.status === 'success' ? {
                        ip: d.query,
                        city: d.city,
                        region: d.regionName,
                        org: d.org,
            ...
            for (const api of apis) {
                try {
                    const data = await tryFetch(api.url, api.parser);
                    if (data) {
                        setIp(data.ip);
                        ...
                        const isVPN = org.includes('vpn') || org.includes('proxy') || org.includes('hosting');
                        if (isVPN) {
                            setVpnWarning(true);
                            setVpnInfo({
                                ip: data.ip,
                                type: org.includes('vpn') ? 'VPN' : 'PROXY',
